"""
This is an adaptation of the ResNet architecture from 2D to 1D based on a hard copy from
https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py (accessed 2020-07-23)
Every 2D layer is replaced with its 1D pendant. Logically, pretraining has been removed.
"""
...
class SimpleResNet(nn.Module):
    """
    This is the basic model architecture used in the experiments.
    """
    def __init__(self,
                 block=BasicBlock,
                 in_features=1,
        ...
        for i in range(len(out_features)):
            ...
            # Check if the residual shortcut can be the identity (with a constant number of features).
            # Otherwise use a conv1x1 as a trainable shortcut to adapt to new number of features.
            if in_channels != out_features[i]:
                shortcut = nn.Sequential(conv1x1(in_channels, out_features[i]),
                                           norm_layer(out_features[i]))
            else:
                # this will build an identity shortcut
                shortcut = None

            # append a residual block
            layers.append(block(inplanes=in_channels, planes=out_features[i], stride=1, downsample=shortcut,
                                norm_layer=norm_layer))
